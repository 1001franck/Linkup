...
async function findById(id) {
	try {
		const { data, error } = await supabase.from('filter_').select('*').eq('id_filter', id).single();
...
async function removeFilter(id) {
	try {
		const { error } = await supabase.from('filter_').delete().eq('id_filter', id);
...
async function createDefaultFilters() {
	...
	try {
		...
		const { error: tableError } = await supabase.from('filter_').select('id_filter').limit(1);
