 ...
 * Cache: 5 minutes (données qui changent peu fréquemment)
 */
async function getGlobalStats() {
	const cacheKey = cache.generateKey('globalStats', {});
	const cached = cache.get(cacheKey);
	if (cached) {
		logger.debug('[getGlobalStats] Données récupérées du cache');
		return cached;
	}
...
async function getCompanyStats() {
	try {
		...
		const { data: companies, error: companiesError } = await supabase.from('company').select(`
				id_company,
				name,
				industry,
				created_at,
		...
		const { data: applications, error: applicationsError } = await supabase.from('apply').select(`
				id_job_offer,
				job_offer!inner (
					id_company
				)
		...
		const companyStats = companies.map((company) => {
			...
			const applicationCount = applications.filter(
				(app) => app.job_offer && app.job_offer.id_company === company.id_company
			).length;
...
async function getTopCompanies(limit = 3) {
	const cacheKey = cache.generateKey('topCompanies', { limit });
	const cached = cache.get(cacheKey);
	if (cached) {
		logger.debug('[getTopCompanies] Données récupérées du cache');
		return cached;
	}
	...
	try {
		...
		const result = enrichedCompanies
...
async function getIndustryStats() {
	try {
		const { data: jobs, error } = await supabase.from('job_offer').select('industry');
...
async function getApplicationStatusStats() {
	try {
		const { data: applications, error } = await supabase.from('apply').select('status');
