			...
			logger.warn(
				{
					...
					body: {
						...req.body,
						password: '[REDACTED]',
						phone: req.body.phone ? `${req.body.phone.substring(0, 5)}...` : 'missing',
					},
...
router.post('/login', authLimiter, async (req, res) => {
	...
	try {
		...
		if (isMobile) {
			logger.info({
				msg: '[LOGIN] Connexion depuis appareil mobile',
...
router.post('/logout', auth(), async (req, res) => {
	try {
		...
		logger.info({ msg: '[LOGOUT] Cookie supprimé avec succès' });
